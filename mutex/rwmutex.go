// Originally copied from https://golang.org/src/sync/rwmutex.go on Jan 5th 2017
//
// Key changes:
// - strip "race" checks
// - return number of active holders
...
func (rw *RWMutex) RLock() {
	...
	if race_Enabled {
		race_Enable()
		race_Acquire(unsafe.Pointer(&rw.readerSem))
	}
}
...
func (rw *RWMutex) RUnlock() {
	if race_Enabled {
		_ = rw.w.state
		race_ReleaseMerge(unsafe.Pointer(&rw.writerSem))
		race_Disable()
	}
...
func (rw *RWMutex) Lock() {
	...
	if race_Enabled {
		race_Enable()
		race_Acquire(unsafe.Pointer(&rw.readerSem))
		race_Acquire(unsafe.Pointer(&rw.writerSem))
	}
}
...
func (rw *RWMutex) Unlock() {
	if race_Enabled {
		_ = rw.w.state
		race_Release(unsafe.Pointer(&rw.readerSem))
		race_Release(unsafe.Pointer(&rw.writerSem))
		race_Disable()
	}
